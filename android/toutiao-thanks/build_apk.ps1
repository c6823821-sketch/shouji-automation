...
& (Join-Path $bt 'aapt2.exe') link -o $unsigned -I $androidJar --manifest (Join-Path $root 'AndroidManifest.xml') --java $gen --min-sdk-version 26 --target-sdk-version 34 --version-code 2 --version-name 1.1 --auto-add-overlay -R $compiled
...
$signed = Join-Path $dist 'toutiao-thanks-v1.1.apk'
